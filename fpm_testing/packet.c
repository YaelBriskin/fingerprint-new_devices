...
static uint32_t thePassword = 0x00000000;
/*!
...
uint8_t checkPassword(void) 
{
	...
	return (packet.data[0] == FINGERPRINT_OK) ? FINGERPRINT_OK : FINGERPRINT_PACKETRECIEVER;
...
uint8_t GetFromUart(fingerprintPacket *packet)
{
	...
	{
		printf("Packet length exceeds buffer size: %d\n", length);
	...
	printf("\npacket: ");
for (int i = 0; i < MIN_SIZE_PACKET + length; i++) {
    printf("%02X ", pData[i]);
}
printf("\n");
	return packet->data[0];
